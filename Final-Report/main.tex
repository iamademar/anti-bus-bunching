...
%% Restyled onto the Elsevier elsarticle journal template (two-column, author--year refs).

\documentclass[final,5p,times,twocolumn,authoryear]{elsarticle}

% ---------- Packages ----------
%% elsarticle already loads graphicx and (via authoryear) natbib.
\usepackage{microtype}  %% better justification in the narrow two-column measure
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{booktabs}
\usepackage{array}
...
\journal{COMPX523-26A: Machine Learning for Data Streams}
...
\author[uow]{Ademar Tutor}
\author[uow]{Zaimei Xing}
\author[uow]{Member~3}
\author[uow]{Member~4}
\author[uow]{Member~5}
\author[uow]{Member~6}

\affiliation[uow]{organization={Department of Computer Science, University of Waikato},
            city={Hamilton},
            country={New Zealand}}
...
\begin{keyword}
bus bunching \sep data stream mining \sep concept drift \sep ADWIN \sep
Adaptive Random Forest \sep prequential evaluation
\end{keyword}
...
The static threshold is a non-learning reference. Throughput (inst./s) is wall-clock and
hardware-dependent, so the absolute values are indicative rather than reproducible; only the
relative ordering (e.g.\ HAT roughly an order of magnitude faster than ARF) is meaningful.}
\label{tab:bakeoff}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\linewidth]{figures/string_diagram_after.png}
